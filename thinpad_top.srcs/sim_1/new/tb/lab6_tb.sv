...
module lab6_tb;
    ...
    parameter BASE_RAM_INIT_FILE = "G:\\Workspaces\\2022F\\vivado\\main.bin"; // BaseRAM 初始化文件，请修改为实际的绝对路�?
    parameter EXT_RAM_INIT_FILE = "G:\\Workspaces\\2022F\\vivado\\eram.bin";  // ExtRAM 初始化文件，请修改为实际的绝对路�?
  
    initial begin
        // 在这里可以自定义测试输入序列，例如：
        dip_sw = 32'h8000_0002;
        touch_btn = 0;
        reset_btn = 0;
        push_btn = 0;
  ...
  // 待测试用户设�?
    ...
    initial begin
        integer fd;
        fd= $fopen("sample_pc_2022.txt", "w");

        forever begin
            @(posedge dut.sys_clk);

            if (dut.wb_inst_pc != 32'h0000_0000) begin
                $fwrite(fd, "%h\n", dut.wb_inst_pc);
            end
        end
